...
void HexVector::play(){   //In this function played from computer.(non -parameter).
...
void HexVector::play(const Cell& State){
     ...
     if(turn == 1 && board[State.row][State.column].cellState != user1 && board[State.row][State.column].cellState != user2){  //I am updating the position for User 2
                    ...
                    record = board[State.row][State.column];
                    Gmove++;
                    print(winFlag);
                    turn = 2;
                }
...
void HexVector::playGame(){  //In this function, we select size,game type in briefly we start the game here.

    int tempSize;
    checkIn();        
    cout <<"<<<<  WELCOME TO THE HEXVECTOR  >>>>" << endl;                      
...
void HexVector::reset(){     //In this function, I made my assignments in the cell class.
...
void HexVector::print(int winFlag){  //I set up this function to print the table with its point, x and letter o.
    char ch1 = 'x';
    char ch2 = 'o';
    int i,j;
    int counter = 1;
...
void HexVector::userGame(){   //Function to get the player input and update the board.
...
void HexVector::writeToFile(const string& filename){ //in this function I save the game information in txt file.
...
void HexVector::readFromFile(const string& filename){ //I load the saved game in the terminal and I use the const reference parameter in this function.
...
int HexVector::winGame_X(int x,int y,int a){  //I used recursive to find the win condition in this function.
...
void HexVector::userComputer(){   //in this function Computer and user enter input to the board.
...
int HexVector::operator()(int x, int y)const{

    return board[x][y].cellState;
}
